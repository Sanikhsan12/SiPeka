...
- **Dashboard Warga**: Warga dapat membuat laporan baru, mengunggah foto bukti, dan memantau perkembangan laporannya secara _real-time_.
...
4. Akses frontend di `http://localhost:80` dan API backend di `http://localhost:5000`.
   _(Catatan: Akun admin bawaan telah dibuat otomatis di database)_
...
4. Access the frontend at `http://localhost:80` and the backend API at `http://localhost:5000`.
   _(Note: A default admin account is automatically seeded into the database)_
...
4. フロントエンドは `http://localhost:80` に、バックエンドAPIは `http://localhost:5000` にアクセスしてください。
   _(注: デフォルトの管理者アカウントは自動的にデータベースに登録されます)_
